...
class AppConfigService {
  ...
  Future<Map<String, String>> getAppConfig() async {
    ...
    final response = await _dio.post(
      '/rpc_api/res/case_last_rank/upload',
      data: encryptedBody,
      options: dio.Options(
        headers: {
  ...
  Future<Map<String, String>> _buildHeaders() async {
    ...
    if (deviceId.isEmpty) {
      deviceId = prefs.getString('device_uuid_v1') ?? '';
    }
    ...
    if (Platform.isIOS) {
      ...
      model = iosInfo.model;
      // keep device-id stable from local uuid
    ...
    if (deviceId.isNotEmpty) {
      await prefs.setString('device_uuid_v1', deviceId);
    }
    ...
    print('AppConfig http_headers: $headers');
